...
public:
     ...
     */
    explicit CustomTestEvent(int data) : GEvent(GEvent::User), m_data(data) {}
 ...
 *
 * Verifies GObject::setObjectName(), GObject::objectName(), GObject::thread(), and GObject::moveToThread().
 */
 ...
 *
 * Verifies GObject::addCleanupCallback() registers callbacks that execute when GObject is destroyed.
 */
 ...
 *
 * Verifies GObject::connect() overload for functor and lambda slots associated with a context object,
 * testing GSignal::emit() and GSignal::operator().
 */
TEST(GObjectTest, LambdaSlotConnection) {
    ...
    GObject::connect(sig, &context, [&receivedValue, &callCount](int val) {
        receivedValue = val;
        callCount++;
    }, G::DirectConnection);
 ...
 *
 * Verifies GSignal<int, std::string, double> correctly passes diverse primitive and object arguments
 * to a receiver slot.
 */
 ...
 *
 * Verifies GObject::removeEventFilter() unregisters an installed event filter and that non-consuming
 * event filters permit event processing.
 */
 ...
 *
 * Verifies GObject::startTimer() returns valid timer IDs and GObject::killTimer() stops active timers.
 */
 ...
 *
 * Verifies that GObject::connect() with G::AutoConnection or G::QueuedConnection routes signal emissions
 * across thread boundaries into the receiver's thread event loop for execution.
 */
TEST(GObjectTest, CrossThreadMemberFunctionConnection) {
    ...
    GObject::connect(sig, &receiver, [&workerThread](int) {
        workerThread.quit();
    }, G::AutoConnection);
...
TEST(GObjectTest, CrossThreadLambdaConnection) {
    ...
    GObject::connect(sig, &context, [&receivedMsg, &executedInThread, &workerThread](std::string msg) {
        receivedMsg = msg;
...
TEST(GObjectTest, ReceiverDestroyedBeforeQueuedEventHandled) {
...

    {
        ...
        GObject::connect(sig, receiver.get(), &GObjectTestReceiver::onValueReceived, G::QueuedConnection);
    ...
    GSignal<> quitSig;
    GObject::connect(quitSig, &dummyContext, [&workerThread]() {
        workerThread.quit();
    }, G::QueuedConnection);
    quitSig.emit();
...
TEST(GObjectTest, ContextDestroyedBeforeQueuedLambdaHandled) {
...

    {
        ...
        GObject::connect(sig, &context, [&lambdaExecuted](int) {
            lambdaExecuted = true;
        }, G::QueuedConnection);
